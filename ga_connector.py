...
class GoogleConnector(InputChannel):
    ...
    def blueprint(self, on_new_message):  # Here, we define the webhook that Google Assistant
        ...
        google_webhook = Blueprint('google_webhook', __name__)  # collect responses and send them to Google Assistant
        ...
        async def receive(request):  # Then we define the main route for our purpose
            ...
            if intent == 'actions.intent.MAIN':  # The initial message we ask to recieve when the assitant is invoked
                message = "Hello! Welcome to Parcel Shipper, your assistant for managing your shipments"
            else:
